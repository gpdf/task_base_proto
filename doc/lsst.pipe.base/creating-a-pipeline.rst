...
Alternatively the elements of the imports list may be be yaml mapping. This
...
subset, then the subset subset definition is not imported.
...
Once one or more pipelines is imported the ``PipelineTask``\ s defined in
...
different `PipelineTask` than that of the imported declaration, then the
...
declaration defined in the imported `Pipeline` dropped.
...
desired camera, or to import from
...
However, in any tool in which a `Pipeline` can be specified, specific sytax
maybe used when specifying the pipeline which will control how the `Pipeline`
will be loaded.
...
the supplied label) and instrument will still be loaded.

Akin to loading a single label, multiple labels may be specified by
separating each one with a comma. As mentioned above, subsets are useful for
specifying multiple labels at one time. As such labeled subsets can be used
in the parameter list to indicate all labels associated with the subset
should be run. Also like labels, multiple labeled subsets can be used by
separating each with a comma. As you can see labeled subsets are essentially
synonymous with labels and can be used interchangeably. As such, it is also
possible to mix task labels and labeled subsets when specifying a parameter
list.
